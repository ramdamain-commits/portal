...
### Security
- 公開リポに `portalVisible:false` の非公開プロジェクト（personal-kb の給与・等級・転職情報、menes 等）の本文が `life-ops-export.json` 経由で露出していた問題を是正
- リポを一時 private 化し、機微情報を除去した上で git 履歴を再構築（過去コミットからも完全除去）
- export 側（`setting/scripts/Invoke-LifeOpsConsole.ps1`）に allowlist 方式の機微情報マスクを実装し、再発防止（公開PJに紐づくデータのみ出力。非公開PJ・横断運用ログは projects 一覧・各セクションから除外）

## 2026-04-15

### Added
- データ更新ボタンにローカル同期サーバー（localhost:8377）連携を追加 (7bf397c)
